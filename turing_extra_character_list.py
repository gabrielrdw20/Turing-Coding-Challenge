'''This module finds an additional character in one of two given lists. Result of a list type.'''
...
def find(list1: list, list2: list) -> str:
    ...
    if isinstance(list1, str) and isinstance(list2, str):
        ...
        if len(list1) > len(list2):
            ...
            result = []
            result.append([x for x in list1 if x not in list2])
            result.append([x for x in list2 if x not in list1])
            result = sum(result, [])
    else:
